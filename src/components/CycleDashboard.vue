...
const API_URL =
  "https://mskian.github.io/strava-running-stats/cycle.json";
...
async function load() {
  ...
  try {
    ...
    if (!data || !data.week || !data.month || !data.year) {
      throw new Error("Invalid cycling stats structure");
    }

    stats.value = data;
  } catch (err) {
    console.error("Cycling stats load failed:", err);
    error.value = "Failed to load cycling statistics.";
  } finally {
